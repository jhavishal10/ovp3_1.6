...
private  String decodeUnicode(String theString) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		*/
		...
		g_cur_ch_type=intent.getStringExtra("cur_ch_type");
		...
		g_multicast_ip=intent.getStringExtra("multicast_ip");
		g_multicast_port=Integer.valueOf(intent.getStringExtra("multicast_port"));
		...
		if (g_cur_ch_type.equals("dvr")|| g_cur_ch_type.equals("delay")||g_cur_ch_type.equals("hls"))
		{
			g_m3u8_channel=1;
		}else
		{
		...
		if (g_m3u8_channel==1)
		{
			...
			g_churl="ch"+g_cur_ch_no;
			//g_url="http://"+g_ezserver_ip+":"+g_http_port+"/ch"+g_cur_ch_no+".m3u8?token="+g_token;
			g_url="http://"+g_ezserver_ip+":"+g_http_port+"/"+g_churl+".m3u8?token="+g_token;
		}else
		{
			g_churl=g_cur_ch_no+".ch";
			g_url="http://"+g_ezserver_ip+":"+g_http_port+"/"+g_churl+"?token="+g_token;
			int keyword_pos=0;
			int crln_pos=0;
			String iplist=null;
			iplist=g_multicast_ip;
			keyword_pos=iplist.indexOf(".");
			if (keyword_pos!=-1) {
				g_multicast_ip1 = Integer.valueOf(iplist.substring(0, keyword_pos));
				iplist=iplist.substring(keyword_pos+1);
				keyword_pos=iplist.indexOf(".");
				if (keyword_pos!=-1) {
					g_multicast_ip2 = Integer.valueOf(iplist.substring(0, keyword_pos));
					iplist=iplist.substring(keyword_pos+1);
					keyword_pos=iplist.indexOf(".");
					if (keyword_pos!=-1) {
						g_multicast_ip3 = Integer.valueOf(iplist.substring(0, keyword_pos));
						iplist = iplist.substring(keyword_pos + 1);
						g_multicast_ip4 = Integer.valueOf(iplist.substring(0));
					}
				}
			}
			g_multicast_ip4 = (g_multicast_ip4 + g_cur_ch_no-1) % 256;
			if (g_multicast_ip4+g_cur_ch_no>255) {
					g_multicast_ip3++;
					if (g_multicast_ip3+g_cur_ch_no>255) {
						g_multicast_ip3 = (g_multicast_ip4 + g_cur_ch_no) % 256;
						g_multicast_ip2++;
						if (g_multicast_ip2+g_cur_ch_no>255) {
							g_multicast_ip2 = (g_multicast_ip4 + g_cur_ch_no) % 256;
							g_multicast_ip1++;
						}
					}
			}
			if (g_multicast_ip1>0) {
				g_url = "udp://@" + g_multicast_ip1 + "." + g_multicast_ip2 + "." + g_multicast_ip3 + "." + g_multicast_ip4 + ":" + g_multicast_port;
			}
		}

		g_message="CH "+g_cur_ch_no;
		Toast.makeText(Ovp.this, g_message, Toast.LENGTH_LONG).show();
		new AsyncTask_Get_CH_information().execute(g_token,g_ezserver_ip,Integer.toString(g_ezserver_port));
		...
		*/
		...
		mEPG_list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> a, View view, int index,long id) {
	    		  	...
	    		  	new AsyncTask_Get_EPG_Day_Info().execute(g_token,g_ezserver_ip,Integer.toString(g_ezserver_port),Integer.toString(g_cur_ch_no),Integer.toString(index-7));
	    		/*
		...
		mEPG_Day_list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> a, View view, int index,long id) {
				...
				if (starmark.equals("[*]"))
				{
					...
					*/
					new AsyncTask_Get_Movie_Duration_Bitrate().execute(g_token,g_ezserver_ip,Integer.toString(g_ezserver_port),szMovieName);
...
//							g_url="http://"+g_ezserver_ip+":"+g_http_port+"/"+g_cur_ch_no+".ch?token="+g_token+":muxer=flv";
							//String ch_type=g_channel_info.GetChannelType(g_cur_ch_no-1);
						   //g_message="1.g_cur_ch_no="+g_cur_ch_no+"ch_type="+ch_type;
						   //Toast.makeText(Ovp.this, g_message, Toast.LENGTH_LONG).show();
						   //String ch_type=g_channel_info.GetChannelType(g_cur_ch_no-1);
						   //g_cur_ch_type=ch_type;
						   String ch_type=g_channel_info.GetChannelType(g_cur_ch_no-1);
						   g_cur_ch_type=ch_type;
						   if (ch_type.equals("dvr")|| ch_type.equals("delay")||ch_type.equals("hls"))
							{
...
//							g_url="http://"+g_ezserver_ip+":"+g_http_port+"/"+g_cur_ch_no+".ch?token="+g_token+":muxer=flv";
                           String ch_type=g_channel_info.GetChannelType(g_cur_ch_no-1);
                           g_cur_ch_type=ch_type;
                           //g_message="2.g_cur_ch_no="+g_cur_ch_no+"ch_type="+ch_type;
						   //Toast.makeText(Ovp.this, g_message, Toast.LENGTH_LONG).show();
						   //String ch_type=g_cur_ch_type;
						   if (ch_type.equals("dvr")|| ch_type.equals("delay")||ch_type.equals("hls"))
							{
								g_m3u8_channel=1;
							}else
...
private void vlc_player(String szURL)
{
		...
		try {
			...
			CHUicodeName=decodeUnicode(UnicodeString);
				...
				if (g_m3u8_channel==1) {
					...
					new AsyncTask_playVideo().execute(path);
				}
			}
		} catch (Exception e) {
			Log.e(TAG, "error: " + e.getMessage(), e);
	...
	private class AsyncTask_Get_Movie_Duration_Bitrate extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... async_input) {
			Get_Movie_Duration_Bitrate(async_input[0],async_input[1],Integer.valueOf(async_input[2]),async_input[3]);
			movie_url=async_input[3];
			return null;
		}
	...
	private boolean Show_Playback_Menu()
	{
		...
		if (ch_type.equals("dvr"))
		{
			...
			new AsyncTask_Get_Channel_DVR_Uptime_Seconds().execute(g_token,g_ezserver_ip,Integer.toString(g_ezserver_port));
			/*
			...
			scheduleTaskExecutor= Executors.newScheduledThreadPool(5);
			    ...
			    queueFuture=scheduleTaskExecutor.scheduleAtFixedRate(new Runnable() {
			      public void run() {
					...
					if (g_channel_dvr_buffer_peroid>0) {
	...
	*/
	private class AsyncTask_Get_Channel_DVR_Uptime_Seconds extends AsyncTask<String, String, String> {
		protected String doInBackground(String... async_input) {
			Get_Channel_DVR_Uptime_Seconds(async_input[0],async_input[1],Integer.valueOf(async_input[2]));
	...
	private class AsyncTask_Get_EPG_Day_Info extends AsyncTask<String, String, String> {
		protected String doInBackground(String... async_input) {
			epg_day_list_array=Get_EPG_Day_Info(async_input[0],async_input[1],Integer.valueOf(async_input[2]),Integer.valueOf(async_input[3]),Integer.valueOf(async_input[4]));
	...
	private class AsyncTask_Get_CH_information extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... async_input) {
			...
			total_ch_no=Get_CH_information(async_input[0],async_input[1],Integer.valueOf(async_input[2]));
	...
	private int Get_CH_information(String token, String ezserver_ip, int ezserver_port){
